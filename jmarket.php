...
$data= '<table class="table table-bordered table-striped" style="width: 70%;"> <tr> <th> Farmer Name </th> <th> Farm Name </th> <th> Rating </th> <th> </th> </tr><tbody id="myTable">';
...
while($row=mysqli_fetch_array($sql))
      {
      $data .='<tr>
      <td style="font-size: 20px;"> <span style="opacity: 0;"> All </span> <img src="farmerpfp/'.$row['Farmer_Pfp'].'" width="200" height="200" alt="No Image" id="alt" class="farm" style="font-size: 15px;"/> 
     '.$row['Username'].' </td>
    <td> '.$row['Farmer_Loc_Name'].' </td>';
  

    $data.='<td style="color:#E3CB22; font-size: 30px;">' .str_repeat("★",$row['Rating']) .'</td>
    <td> <button type="submit" class="btn btn-success" data-dismiss="modal" value="'.strval($row['Username']).'" name="rate"><a href="jrating.php?"><span style="color: white;text-decoration: none;">Rate me </span></a></button> </td>
   </tr>';
      }
    }
  $data .='</tbody> </table>';
