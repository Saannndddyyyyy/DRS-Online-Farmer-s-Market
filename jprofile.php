...
<center>
 ...
 <?php
  $data= '<div class="card text-center box-card-set" style="box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19); width: 30%; float:left; margin-left: 500px;">';
    ...
    $displayquery = mysqli_query($con, "SELECT * FROM iwp_database___user_database  WHERE Username='$username'");
    ...
    <div class="col-6"> <B> Address:</B> ' .$row['User_Loc_Name'].'</div> 
    <div class="col-6"> <B> Phone No.: </B>' .$row['User_Phone'].'</div>';
...
<h2 style="font-size: 25px;"> Current Profile Picture </h2>
  <?php
$con=mysqli_connect("localhost","root","","OFM");
    ...
    $query1 = mysqli_query($con,"SELECT User_Pfp FROM iwp_database___user_database WHERE Username='$username'");
  while($row = mysqli_fetch_array($query1))
{
$image=$row['User_Pfp'];
$image_src="userpfp/".$image;}
...
else
{
  ...
  $query = mysqli_query($con,"UPDATE iwp_database___user_database SET User_Pfp='$name' WHERE Username='$username'");
...
?>
   ...
   <form action="jprofile.php" method="POST" enctype="multipart/form-data">
