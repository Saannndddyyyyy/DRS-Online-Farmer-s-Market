...
<html>
<head>
  <title>Drs Select Payment</title>
...
style=" fill:#000000;"><g fill="none" fill-rule="nonzero" stroke="none" stroke-width="1" stroke-linecap="butt" stroke-linejoin="miter" stroke-miterlimit="10" stroke-dasharray="" stroke-dashoffset="0" font-family="none" font-weight="none" font-size="none" text-anchor="none" style="mix-blend-mode: normal"><path d="M0,172v-172h172v172z" fill="none"></path><g fill="#ffffff"><path d="M16.6569,28.66667c-9.11652,0 -16.6569,7.54038 -16.6569,16.6569v88.51953c0,9.11652 7.54038,16.6569 16.6569,16.6569h138.6862c9.11652,0 16.6569,-7.54225 16.6569,-16.6569v-88.51953c0,-9.11652 -7.54225,-16.6569 -16.6569,-16.6569zM16.6569,43h138.6862c1.36302,0 2.32357,0.94809 2.32357,2.32357v12.00977h-143.33333v-12.00977c0,-1.37548 0.94809,-2.32357 2.32357,-2.32357zM14.33333,78.83333h143.33333v55.00976c0,1.36302 -0.94809,2.32357 -2.32357,2.32357h-138.6862c-1.37548,0 -2.32357,-0.94809 -2.32357,-2.32357zM28.66667,93.16667v7.16667h50.16667v-7.16667z"></path></g></g></svg><h2 style="color: white;"> Select Card </h2>
    <form name="payment-form" method="post" id="payment-form">
      <table bgcolor="black">
        ...
        <tr>
     <label for="slt"> <?php 
      $query=mysqli_query($con, "SELECT Name_On_Card, Card_Number FROM payments WHERE username='$username'");
        if(mysqli_num_rows($query) >0 )
      {
      while($row1=mysqli_fetch_array($query))
      {   
          $Name=$row1['Name_On_Card'];
          
          $cardno=$row1['Card_Number'];
          
      echo '<tr><td>';
      echo '<input type="radio" id="slt" name="cardselect" value="';
      echo $cardno;
      echo '"></td><td>';
      echo $Name;
      echo '</td>';?></label>
 <td><label for="Number"><?php echo $cardno; echo '</td>'; }} ?></label></td>
    </tr></table>
    <br> <div id="no" style="color: white;"> </div><br>
    <input type="submit" class="btn btn-primary custom" name="pay" value="Submit" id="submit_form">
    <br> <br>
...
<?php 
  ...
  {
    $_SESSION['cnum']=$_POST['cardselect'];
  }
    if(isset($_POST['pay']))
    {
      if(isset($_POST['cardselect']))
      echo '<script> window.location="confirmation2.php"</script>';
    else
      echo '<script> document.getElementById("no").innerHTML="Select a Card!"</script> <br>';
    }
